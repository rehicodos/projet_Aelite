<?php
    ...
    function insertion_devis_item(){

        if(isset($_POST['valider_devis_general'])){

            if(count($_POST['designation']) > 1){
    ...
    class Invoice{
        ...
        public function saveInvoice($POST) {		
            ...
            for ($i = 0; $i < count($POST['designation']); $i++) {
                $sqlInsertItem = "
                INSERT INTO ".$this->invoiceOrderItemTable."(ord_id, designation, unite, quantite_, prix_u, prix_t) 
                VALUES ('".$lastInsertId."', '".$POST['designation'][$i]."', '".$POST['unite'][$i]."', '".$POST['quantite'][$i]."', '".$POST['prix_u'][$i]."', '".$POST['prix_t'][$i]."')";			
